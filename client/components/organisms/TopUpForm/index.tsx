...
export default function TopUpForm(props :TopupFormProps) {
  const [verifyID, setVeriyID] = useState('');
  const [bankAccountName, setBankAccountName] = useState('');
  const [nominalItem, setNominalItem] = useState({});
  const [paymentItem, setPaymentItem] = useState({});
  const router = useRouter();
  const onNominalItemChange = (nominal:NominalsTypes) => {
  ...
  const onSubmit = () => {
    ...
    if (verifyID === '' || bankAccountName === '' || nominalItem === ({}) || paymentItem === ({})) {
      ...
      const localName = btoa('data-topup');
      localStorage.setItem(localName, hash);
      ...
      <div className="pt-md-50 pt-30">
        <div className="">
          ...
          <input
            ...
            value={verifyID}
            onChange={(event) => setVeriyID(event.target.value)}
          />
        </div>
      </div>
