...
export default function SignUpPhoto() {
  ...
  const getGameCategoryAPI = useCallback(async () => {
    ...
    setCategories(data);
    setFavorite(data[0]._id); // to set first id category when render
  }, [getGameCategory]);
  ...
  useEffect(() => {
    ...
    setLocalForm(JSON.parse(getLocalForm!)); // ! untuk memastikan bahwa data localForm selalu ada
  }, []);

  const onSubmit = async () => {
    const getLocalForm = localStorage.getItem('user-form');
    const form = await JSON.parse(getLocalForm!);
