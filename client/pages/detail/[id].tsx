...
export default function Detail() {
  ...
  const getVoucherDetailAPI = useCallback(async (id:any) => {
    ...
    if (result.error) {
      ...
      setDataItem(result.data.voucher);
      const key = 'result@_d4tavoucher';
      const hash = CryptoJS.AES.encrypt(JSON.stringify(result.data.voucher), key).toString();
      localStorage.setItem(btoa('data-item'), hash);
