...
module.exports = {
  ...
  detailPage: async (req, res) => {
    try {
      ...
      if (!voucher) {
        return res.status(404).json({ message: 'voucher tdak ditemukan' });
      }
    ...
    try {
      ...
      const payload = {
        ...
        historyUser: {
          ...
          phoneNumber: res_voucher._doc.phoneNumber?._id
        },
  ...
  historyDetail: async (req, res) => {
    try {
      ...
      if (!history)
        return res.status(400).json({ message: 'history tidak ditemukan' });
